...
def seleccionar_imagen():
    root = Tk()
    root.filename = filedialog.askopenfilename(initialdir="/", title="Seleccionar archivo", filetypes=(("Archivos de imagen", "*.jpg *.jpeg *.png *.gif"), ("Todos los archivos", "*.*")))
    root.destroy()  # Cerrar la ventana después de seleccionar el archivo
    return root.filename

def dividir_imagen(nombre_archivo):
    # Leer la imagen seleccionada (en la ruta) para guardarla
    with open(nombre_archivo, 'rb') as file:
        imagen_bytes = file.read()

    # Calcular la longitud total de la imagen
    longitud_total = len(imagen_bytes)

    # Calcular el índice de división
    indice_division = longitud_total // 2

    # Dividir la imagen en dos partes
    parte1 = imagen_bytes[:indice_division]
    parte2 = imagen_bytes[indice_division:]
...
def guardar_mensaje():
    ...
    if response.status_code == 200:
        ...
        # Llamar a la función para seleccionar una imagen
        nombre_completo = seleccionar_imagen()
        ...
        # Leer la imagen seleccionada (en la ruta) para guardarla
        parte1, parte2 = dividir_imagen(nombre_completo)


        for indice, dataNode in enumerate(lista_de_data_nodes):
            if indice == 0:
                response = requests.post(f'http://{dataNode["host"]}:{dataNode["port"]}/guardar', json={'archivo': {'nombre': nombre_con_fecha_hora, 'imagen': parte1}})
                if response.status_code == 200:
                    print(response.text)
                ...
                response = requests.post(f'http://{dataNode["host"]}:{dataNode["port"]}/guardar', json={'archivo': {'nombre': nombre_con_fecha_hora, 'posicion': 2, 'imagen': parte2}})
                if response.status_code == 200:
                    print(response.text)
